...
首先是一个统计Cells个数和范围的函数，关键代码如下

```c++
void generateGridDimensions(
    const DataPointsType &dataPoints,//数据集
    const float &epsilon,
    MinMaxValsType &minVals,//每一维上最小值
    MinMaxValsType &maxVals,//每一维上最大值
    array<uint, 2> &nCells,//记录每一维上可以分成多少个Cells
    uint64 &totalCells) {
...
合并的时候，我们需要在shadowCells数据上进行半个聚类算法，判断不同子块之间
