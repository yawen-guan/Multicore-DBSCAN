...
可见，随着blockSize的从4倍增到32，基线（Original DBSCAN）不受影响，Hybrid DBSCAN的运行时间大致减少 ，相较于基线的加速比随着数据量的增加可以有指数级增加，最高可达400~500。BPS DBSCAN的表现类似，但是不及Hybrid DBSCAN运行那么快，加速比最多只有14。

出现这种现象的原因是blocksize变大时GPU并行效果增强，而且我们实现的算法在数据很大的时候加速效果十分显著。
...
可见，随着blockSize的从4倍增到32，基线（Original DBSCAN）不受影响，Hybrid DBSCAN的运行时间先上升再下降。BPS DBSCAN的表现类似，但是不及Hybrid DBSCAN运行那么快，加速比最多只有16。

出现这种情况的原因是当NCHUNKS比较小的时候，分块并行带来的加速效果不能弥补分块操作的额外开销，所以运行效率稍有下降；但NCHUNKS变大之后分块并行的加速效果就体现了出来。
