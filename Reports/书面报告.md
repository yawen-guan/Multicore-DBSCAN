...
<img src="./asset/DBSCAN-effect.svg" alt="DBSCAN-effect" style="zoom: 50%;" /><img src="./asset/DBSCAN-Illustration.svg" alt="DBSCAN-Illustration" style="zoom:100%;" />
...
<img src="asset/Partition.png" alt="Partition" style="zoom: 40%;" />
...
```c++
void generatePartitions(const DataPointsType &dataPoints,/* 数据集 */vector<uint> &binBounaries,/* 记录边界是在哪个Cells上 */ /* 其他参数省略 */const int &NCHUNKS) {
...
```c++
void generatePartitionDatasets(/* 其他参数省略 */ vector<DataPointsType> &partDataPointsArray,/* 分割之后多组小数据集 */ vector<PointChunkLookup> &pointChunkMapping,/* 分割前分割后下标映射 */ vector<uint> &pointIDs_shadow, /* shadowCells的下标映射 */ DataPointsType &dataPoints_shadow /* shadowCells里的数据点，可以看做独立的一个数据集 */){
...
- 合并不相邻Grid
  ...
  void DenseBox::mergeNonAdjacent() {
      // 计算步长
      auto stepList = vector<int>({/* 不相邻的Grid，细节省略 */});
      vector<bool> isVisited(_totalCells, false);
      for (auto i = 0; i < _totalCells; ++i) {
          // 合并不相邻的Grid
      }
...
- 合并$C^{DBox}$和$C^{DBSCAN}$
  ...
  void DenseBox::mergeAll() {
      auto stepList = vector<int>({/* 需要合并的格子,细节省略 */});
      vector<bool> isVisited(_totalCells, false);
      for (auto i = 0; i < _totalCells; ++i) {
          // 合并DenseBox的聚类和DBSCAN的聚类
      }
  }
  ```
  ...
  void DenseBox::reArrangeCluster() {
      ...
      for (auto i = 0; i < _curClusterNums; ++i) {
          if (clusterIDMap[clusterID[i].second] > _minpts)
              clusterIDMap[clusterID[i].first] = i; // 重排聚类
          else
              clusterIDMap[clusterID[i].first] = 0; // 去除噪音
      }
...
<img src="./asset/Original-DBSCAN-data-2500-out.png" alt="Original-DBSCAN" style="zoom:30%;" /><img src="./asset/Hybrid-DBSCAN-data-2500-out.png" alt="Hybrid-DBSCAN" style="zoom:30%;" /><img src="./asset/BPS-DBSCAN-data-2500-out.png" alt="BPS-DBSCAN" style="zoom:30%;" />
